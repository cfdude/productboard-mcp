...
function generateAxiosCall(operation: string, requiredParams: string[]): string {
  ...
  if (pathParams) {
    pathParams.forEach(param => {
      ...
      interpolatedPath = interpolatedPath.replace(param, `\${args.${paramName}}`);
    });
  }
  
  // Build query params
  const queryParams = requiredParams.filter(p => 
  ...
  // Generate axios call
  if (["post", "put", "patch"].includes(lowerMethod) && requiredParams.includes("body")) {
    return `${paramsCode}
...
function generateToolImplementation(
  ...
  const axiosCall = generateAxiosCall(toolInfo.operation, toolInfo.requiredParams);
...
function generateCategoryFile(category: string, tools: Record<string, ToolInfo>): string {
  const categoryTools = Object.entries(tools).filter(([_, info]) => info.category === category);
...
`;
  ...
  switch (name) {
${toolDefinitions.map(tool => `    case "${tool.name}":
      return await ${tool.functionName}(args);`).join("\n")}
    default:
...
`;
  ...
  const implementations = categoryTools.map(([toolName, toolInfo]) => {
    const functionName = toolNameToFunctionName(toolName);
...
function main() {
  try {
    ...
    const existingCategories = ["notes", "features", "companies", "users", "releases", "webhooks"];
    ...
    Object.entries(manifest.categories).forEach(([categoryId, category]) => {
      ...
      if (content) {
        ...
        console.log(`✅ Generated ${categoryId}.ts with ${category.tools.length} tools`);
        generatedCount++;
      }
    });
