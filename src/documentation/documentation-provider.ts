...
export class DocumentationProvider {
  ...
  private getToolHelpPrompt(toolName: string): GetPromptResult {
...
Please check the tool name and try again.`,
  ...
  private getCategoryGuidePrompt(category: string): GetPromptResult {
    const categoryDoc =
...
`
  ...
  private getWorkflowPrompt(workflow?: string): GetPromptResult {
...
`
  ...
  private getParameterHelpPrompt(parameter?: string): GetPromptResult {
    if (!parameter) {
...
${paramDoc.examples.map((ex: any) => `- **${ex.value}**: ${ex.useCase}`).join('\n')}`;
  ...
  private getQuickStartPrompt(): GetPromptResult {
