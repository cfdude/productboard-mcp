...
export class ToolRegistry {
  ...
  async registerFromManifest(): Promise<void> {
    ...
    for (const [toolName, toolInfo] of Object.entries(this.manifest.tools)) {
      ...
      if (this.enabledCategories.size > 0 && !this.enabledCategories.has(toolInfo.category)) {
        continue;
      }
      ...
      this.registerLoader(toolName, async () => {
        try {
          ...
          if (existingCategories.includes(toolInfo.category)) {
            ...
            if (!handler) {
              throw new Error(`Handler ${handlerName} not found in ${toolInfo.category}.js`);
            }
            ...
            const categoryFile = toolInfo.category.replace(/\s/g, "").toLowerCase();
            const module = await import(`../../generated/tools/${categoryFile}.js`);
            
            // Get the handler function name
            const handlerName = `handle${toolInfo.category.split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join("")}Tool`;
            const handler = module[handlerName];
    ...
    for (const [toolName, toolInfo] of Object.entries(this.manifest.tools)) {
      ...
      definitions.push({
        ...
        inputSchema: {
          type: "object",
          properties,
          required: toolInfo.requiredParams.length > 0 ? toolInfo.requiredParams : undefined
        }
      });
    }
